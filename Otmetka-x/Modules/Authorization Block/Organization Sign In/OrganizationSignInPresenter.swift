//
//  OrganizationSignInPresenter.swift
//  Otmetka-x
//
...
protocol OrganizationSignInPresenterProtocol: AnyObject {
    func setup()
...
final class OrganizationSignInPresenter: ModuleProtocol, OrganizationSignInPresenterProtocol {
    
    // MARK: - Module
    
    typealias InputType = OrganizationSignInModuleInput
    typealias OutputType = OrganizationSignInModuleOutput
    
    var output: ((OutputType) -> Void)?
    
    // Dependencies
    let interactor: OrganizationSignInInteractorProtocol
    private weak var view: OrganizationSignInViewControllerDelegate?
    ...
    init(interactor: OrganizationSignInInteractorProtocol,
         view: OrganizationSignInViewControllerDelegate) {
    ...
    func doneButtonDidTap() {
        ...
        Task {
            await MainActor.run {
                interactor.organizationSignIn(with: newParameters)
            }
        }
    }
